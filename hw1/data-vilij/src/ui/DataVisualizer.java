...
public final class DataVisualizer extends ApplicationTemplate {
    ...
    public static String regexString = "^@[^\\s]+[^\\s]*[\\s]+(-?)[^\\s]+[\\s]+(-?)?([0-9]*[.])?[0-9]+,(-?)+?([0-9]*[.])?[0-9]+\\s*$\t(?<=@)[^\\s]*(?!:(\\s*[^\\s]*[+-]?([0-9]*[.])?[0-9]+,[+-]?([0-9]*[.])?[0-9]+))\t(?<=[^\\s]{1,20})\\s+[^\\s]*(?=(\\s+(?<=\\s{1,4})[+-]?([0-9]*[.])?[0-9]+,[+-]?([0-9]*[.])?[0-9]+))\t(?<=[^\\s])\\s+(-?)?([0-9]*[.])?[0-9]+(?<!,\\d)\t(?<=[a-zA-Z0-9]\\s{1,20}(-?)?([0-9]{1,4}[.])?[0-9]{1,4},)(-?)([0-9]*[.])?[0-9]+\t";
