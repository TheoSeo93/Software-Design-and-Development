...
public class RandomClusterer extends Clusterer {
    ...
    public RandomClusterer(DataSet dataset, int maxIterations, int updateInterval, int numberOfClusters, boolean isContinuous) {
    ...
    public void run() {
        ...
        if (isContinuous.get()) {
            ...
            dataset=temp;
            initializeCentroids();
            timeline.setCycleCount(maxIterations/updateInterval);
            timeline.getKeyFrames().add(keyFrame);
            timeline.play();
            timeline.setOnFinished(e -> {
                System.out.println(dataset.getLocations().size());
    ...
    private void assignLabels() {
        ...
        dataset.getLocations().forEach((instanceName, location) -> {
            ...
            for (int i = 0; i < centroids.size(); i++) {
                double distance = Math.random()*Double.MAX_VALUE;
